...
import { underlineEachCharCSS } from "./master.js";
...
await underlineEachCharCSS(".codeInput", loginForm.maxlength, 0.5, "0.1em", "var(--background)");
await setupLoginDialog(entryDialog, loginButton, loginForm);
await setupLoginDialog(entryDialog, signupButton, signupForm);

async function constructForm(dialog, form, input) {
    ...
    input.setAttribute("class", `${form.class} monospace blackWhite`);
}

async function setupLoginDialog(dialog, trigger, form) {
    ...
    trigger.addEventListener("click", async () => {
        await constructForm(dialog, form, inputField);
        dialog.showModal();
    });
